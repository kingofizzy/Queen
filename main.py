...
async def is_admins(chat_id: int):
    return [
        member.user.id
        async for member in Bad.get_chat_members(
...
@Bad.on_message(filters.incoming & filters.private, group=-1)
...
@Bad.on_message(filters.command(["start",f"start@{BOT_USERNAME}"]))
...
@Bad.on_message(filters.command(["help", f"help@{BOT_USERNAME}"], prefixes=["/"]))
async def restart(client, message):
    hmm = await message.reply_photo(START_IMG,
                             caption= HELP_READ,
                        reply_markup= InlineKeyboardMarkup(HELP_BACK),
       )
@Bad.on_message(filters.command(['source', 'repo']))
async def source(bot, m):
    await m.reply_photo(START_IMG, caption=SOURCE_TEXT, reply_markup=SOURCE_BUTTONS, reply_to_message_id=m.id)
#  alive
@Bad.on_message(filters.command(["ping","alive"], prefixes=["/"]))
...
async def vickai(client: Client, message: Message):
   ...
   if not message.reply_to_message:
       ...
       is_vick = vick.find_one({"chat_id": message.chat.id})
       if not is_vick:
           await Bad.send_chat_action(message.chat.id, ChatAction.TYPING)
           K = []  
